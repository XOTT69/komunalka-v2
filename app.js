...
async function loadUserSettings() {
    try {
        ...
        if (snap.exists()) {
            // Merge existing db settings with defaults to avoid undefined errors
...
document.getElementById('save-settings-btn').addEventListener('click', async () => {
    ...
    try {
        ...
        renderInputsBasedOnSettings();
        updateAppUI();
        setTimeout(() => { btn.textContent = "Зберегти налаштування"; btn.classList.replace('bg-green-500', 'bg-gray-900'); }, 2000);
...
async function updateAppUI() {
    ...
    // Заповнюємо підказки "Попередній"
    const prevText = pastReadings ? `Попередній (${formatMonthUI(pastReadings.date)}): ` : "Попередній: Немає ";
    
    document.getElementById('last-electro').textContent = prevText + (pastReadings ? pastReadings.electroReading : "");
    ...
    document.getElementById('last-water').textContent = prevText + (pastReadings ? pastReadings.waterReading : "");
    document.getElementById('last-gas').textContent = prevText + (pastReadings ? pastReadings.gasReading : "");
...
document.getElementById('save-btn').addEventListener('click', async () => {
    ...
    // Рахуємо споживання
    const consumedElectro = SETTINGS.features.twoZone ? 0 : (pastReadings && electroReading > 0 ? Math.max(0, electroReading - pastReadings.electroReading) : 0);
    ...
    const consumedWater = pastReadings && waterReading > 0 ? Math.max(0, waterReading - pastReadings.waterReading) : 0;
    const consumedGas = pastReadings && gasReading > 0 ? Math.max(0, gasReading - pastReadings.gasReading) : 0;
    ...
    const data = {
        ...
        electroDay: consumedElectroDay,
        electroNight: consumedElectroNight,
        water: consumedWater,
        gas: consumedGas,
        
        // Абсолютні показники для наступного місяця
        electroReading: electroReading > 0 ? electroReading : (pastReadings ? pastReadings.electroReading : 0),
        electroDayReading: electroDayReading > 0 ? electroDayReading : (pastReadings ? pastReadings.electroDayReading : 0),
        electroNightReading: electroNightReading > 0 ? electroNightReading : (pastReadings ? pastReadings.electroNightReading : 0),
        waterReading: waterReading > 0 ? waterReading : (pastReadings ? pastReadings.waterReading : 0),
        gasReading: gasReading > 0 ? gasReading : (pastReadings ? pastReadings.gasReading : 0),
    ...
    try {
        ...
        setTimeout(() => { btn.textContent = "Зберегти дані"; btn.classList.replace('bg-green-500', 'bg-blue-600'); }, 2000);
        updateAppUI();
...
function calculateCostForRecord(r) {
    ...
    if (SETTINGS.features.twoZone) {
        ...
        if (isWinter && totalKw <= 2000) {
            // Пільговий розрахунок для двозонного
            electroCost = ((r.electroDay || 0) * 2.64) + ((r.electroNight || 0) * 1.32);
        } else {
            // Стандартний розрахунок двозонного
            electroCost = ((r.electroDay || 0) * baseTariff) + ((r.electroNight || 0) * nightTariff);
        }
    } else {
        if (isWinter && r.electro <= 2000) electroCost = r.electro * 2.64;
        else electroCost = r.electro * baseTariff;
    }

    const waterCost = r.water * SETTINGS.tariffs.water;
    const gasCost = r.gas * SETTINGS.tariffs.gas;
...
async function loadHistory() {
    ...
    try {
        ...
        // Шукаємо попередні записи для відображення детальної історії (різниці)
        records.forEach((d, index) => {
            const prev = records[index + 1]; // Запис за попередній місяць
            const total = calculateCostForRecord(d);
            
            // Будуємо HTML для електроенергії
            let electroHtml = '';
            if (SETTINGS.features.twoZone) {
                ...
                electroHtml = `
                    <div class="flex justify-between items-center bg-gray-50 p-2 rounded-lg mb-2">
                        ...
                        <span class="text-xs text-gray-600">${elDiff}<b class="text-gray-900">${d.electroReading||0}</b> = <b class="text-blue-600">${d.electro} кВт</b></span>
                    </div>`;
            }
            ...
            const card = `
                <div class="bg-white p-5 rounded-3xl shadow-sm border border-gray-100">
                    ...
                    <div class="flex justify-between items-center bg-gray-50 p-2 rounded-lg mb-2">
                        <span class="text-xs font-bold text-blue-400"><i class="fa-solid fa-droplet"></i> Вода</span>
                        <span class="text-xs text-gray-600">${waterDiff}<b class="text-gray-900">${d.waterReading||0}</b> = <b class="text-blue-600">${d.water} м³</b></span>
                    </div>` : ''}
                    ${SETTINGS.features.showGas ? `
                    <div class="flex justify-between items-center bg-gray-50 p-2 rounded-lg">
                        <span class="text-xs font-bold text-orange-400"><i class="fa-solid fa-fire"></i> Газ</span>
                        <span class="text-xs text-gray-600">${gasDiff}<b class="text-gray-900">${d.gasReading||0}</b> = <b class="text-blue-600">${d.gas} м³</b></span>
                    </div>` : ''}
                </div>
            `;
...
function checkReminders() {
    ...
    // Функція парсингу "1-5" або "10-15"
    const isTodayInRange = (rangeStr) => {
        if (!rangeStr) return false;
        const parts = rangeStr.split('-');
        if (parts.length === 2) {
            return today >= parseInt(parts[0]) && today <= parseInt(parts[1]);
        }
    ...
    if (toRemind.length > 0) {
        ...
        container.innerHTML = `
            <div class="bg-red-50 border-l-4 border-red-500 p-4 rounded-xl flex items-center gap-3 shadow-sm animate-pulse">
                ...
                <div>
                    <div class="font-bold text-red-800 text-sm">Час передати показники!</div>
                    <div class="text-xs text-red-600">Сьогодні потрібно передати: <b>${msg}</b>.</div>
                </div>
            </div>`;
        
        // Викликаємо системне сповіщення, якщо є дозвіл
        if (Notification.permission === "granted") {
            new Notification("Комуналка PRO: Передайте показники", {
                body: `Сьогодні необхідно передати показники для: ${msg}.`,
...
// Запит на системні пуш-сповіщення (в браузері/телефоні)
    ...
    if (Notification.permission === "default") {
        ...
        notifyBtn.onclick = () => {
            Notification.requestPermission().then(p => {
                if (p === "granted") notifyBtn.classList.add('hidden');
            });
        };
    }
}
